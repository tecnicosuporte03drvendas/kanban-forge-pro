...
Deno.serve(async (req) => {
  ...
  try {
    const { taskId, removedResponsibles } = await req.json();
    
    console.log('notify-task-responsible-removed triggered', { taskId, removedResponsibles });
    ...
    // Processar responsáveis removidos
    const responsiblesData = [];

    for (const respId of removedResponsibles) {
    ...
    const payload = {
      action: 'task_responsible_removed',
    ...
    await supabase.from('instancias_whatsapp_logs').insert({
      acao: 'notificacao_responsavel_removido',
      sucesso: true,
      dados_entrada: { taskId, removedResponsibles },
    ...
    console.error('Erro em notify-task-responsible-removed:', error);
