...
Deno.serve(async (req) => {
  ...
  try {
    console.log('notify-task-reminder-1day triggered');
    ...
    const { data: config } = await supabase
      ...
      .select('valor')
      .eq('chave', 'notif_lembrete_1dia_ativa')
      .single();

    if (!config || config.valor !== 'true') {
      console.log('Notificação de lembrete 1 dia está desativada');
      return new Response(
    ...
    const { data: tarefas } = await supabase
      .from('tarefas')
      .select('*')
      .eq('data_conclusao', tomorrowStr)
      .in('status', ['criada', 'aceita', 'executando'])
      .eq('arquivada', false);

    console.log(`Encontradas ${tarefas?.length || 0} tarefas com vencimento amanhã`);
    ...
    for (const tarefa of tarefas) {
      ...
      if (response.ok) {
        ...
        await supabase.from('instancias_whatsapp_logs').insert({
          acao: 'notificacao_lembrete_1dia',
          sucesso: true,
    ...
    console.error('Erro em notify-task-reminder-1day:', error);
