...
import { LayoutDashboard, CheckSquare, Calendar, BarChart3, Users, User, Settings, HelpCircle, Shield, Moon, Sun, Monitor, LogOut, UserCircle } from "lucide-react";
...
const getMenuItems = (tipoUsuario: string, isStealthMode: boolean, empresaId?: string) => {
  ...
  const stealthParams = isStealthMode && empresaId ? `?stealth=true&master_id=${new URLSearchParams(window.location.search).get('master_id')}` : '';
  const allItems = [{
    title: "Dashboard",
    url: `${baseUrl}/${stealthParams}`,
    icon: LayoutDashboard
  }, {
    title: "Tarefas",
    url: `${baseUrl}/tarefas${stealthParams}`,
    icon: CheckSquare
  }, {
    title: "Agenda",
    url: `${baseUrl}/calendario${stealthParams}`,
    icon: Calendar
  }, {
    title: "Relatórios",
    url: `${baseUrl}/relatorios${stealthParams}`,
    icon: BarChart3
  }, {
    title: "Empresa",
...
export function AppSidebar() {
  ...
  useEffect(() => {
    if (usuario) {
      ...
      const iniciais = nomes.length >= 2 ? `${nomes[0][0]}${nomes[nomes.length - 1][0]}`.toUpperCase() : nomes[0].substring(0, 2).toUpperCase();
  ...
  return <Sidebar className={`${collapsed ? "w-40" : "w-64"} transition-all duration-300 border-r border-sidebar-border bg-sidebar`} collapsible="icon">
          ...
          {!collapsed && <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem asChild>
                  <NavLink to={isStealthMode && empresaId ? `/empresa/${empresaId}/perfil?stealth=true&master_id=${new URLSearchParams(window.location.search).get('master_id')}` : "/perfil"} className="flex items-center cursor-pointer">
