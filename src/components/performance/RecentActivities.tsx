...
export const RecentActivities = ({
  ...
  const loadRecentActivities = async () => {
    try {
      let query = supabase.from('tarefas_atividades').select(`
          id,
          acao,
          descricao,
          created_at,
        ...
        `).eq('tarefas.empresa_id', usuario?.empresa_id).order('created_at', {
        ascending: false
      }).limit(2);
      if (userId) {
    ...
    return <Card className="border-border bg-card">
        ...
        <CardContent>
          <div className="space-y-3 animate-pulse">
            {[...Array(3)].map((_, i) => <div key={i} className="flex items-start gap-3 p-3 border-l-2 border-muted bg-muted/20 rounded-r-lg">
  ...
  return <Card className="border-border bg-card">
      ...
      <CardContent>
        {activities.length === 0 ? <p className="text-muted-foreground text-center py-4">
            Nenhuma atividade recente encontrada.
          </p> : <div className="space-y-3">
            {activities.map(activity => {
          const IconComponent = getActivityIcon(activity.acao);
          const iconColor = getActivityColor(activity.acao);
          return <div key={activity.id} className="flex items-start gap-2 p-2 border-l-2 border-primary/20 bg-accent/30 rounded-r-lg">
                  <IconComponent className={`w-5 h-5 ${iconColor} mt-0.5`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="font-medium text-sm">{activity.acao}</p>
                      {!userId && <Badge variant="outline" className="text-xs">
                          {activity.usuario_nome}
                        </Badge>}
                    </div>
                    {activity.tarefa_titulo && <p className="text-sm text-foreground mb-1 truncate">
                        {activity.tarefa_titulo}
                      </p>}
                    {activity.descricao && <p className="text-xs text-muted-foreground mb-1">
                        {activity.descricao}
                      </p>}
