...
import { MessageSquare, Activity, List } from "lucide-react"; // Added List icon for tasks
...
import type { TarefaComentario, TarefaAtividade, Tarefa } from "@/types/task";
    ...
    <div className="border-l border-border bg-muted/10 flex flex-col h-full">
      <div className="p-4 border-b border-border flex-shrink-0">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-3">
            {" "}
            {/* Updated to 3 columns for tasks */}
      ...
      <div className="flex-1 min-h-0">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          {/* Activity Tab */}
          <TabsContent value="activity" className="h-full mt-0 data-[state=active]:flex flex-col">
            <div className="flex-1 min-h-0">
              <ScrollArea className="h-full">
                <div className="p-4 space-y-4">
                  {activities.length === 0 ? (
                    ...
                    tasks.map((tarefa) => (
                      <div key={tarefa.id} className="flex items-start gap-3">
                        ...
                        <div className="flex-1 space-y-1 min-w-0">
                          ...
                          <div className="text-sm bg-background border rounded-lg p-3 shadow-sm">{tarefa.titulo}</div>
                        </div>
                      </div>
                    ))
                  )}
